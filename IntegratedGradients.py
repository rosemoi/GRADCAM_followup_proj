...
class integrated_gradients:
    ...
    '''
    def explain(self, sample, outc=0, reference=False, num_steps=50, verbose=0, noise=False, smooth=False):
        ...
        if K.backend() == "tensorflow":
            if noise != False:
                gradients_noise = []
                _input_std_0 = np.std(_input[0])**2
                #_input_std_0 = 0.15 * (np.max(_input[0])-np.min(_input[0]))
                _input_std_1 = np.std(_input[1])**2
                #_input_std_1 = 0.15 * (np.max(_input[1])-np.min(_input[1]))
                for i in range(noise):
                    _input_noise_0 = _input[0] + np.random.normal(0,_input_std_0,size=_input[0].shape)
                    _input_noise_1 = _input[1] + np.random.normal(0,_input_std_1,size=_input[1].shape)
                    _input_noise = [_input_noise_0,_input_noise_1,0]
                    gradients_noise.append(self.get_gradients[outc](_input_noise))
                gradients = np.mean(gradients_noise, axis=0)
            else:
